...
### ZSS Lexer (New)
* Using Ekopocalypse's "enhanceanylexer plug-in" we can expand on notepad++'s UDL limitations; and create custom lexers for some ZSS triggers, functions, stcrls....
* Current Features:
- Enhance maps, consts, localvars, some triggers such as hitdefattr, hitbyattr etc... (Will be expanded in the future)
...
currently we have, ";" check for stcrls/localvars, invalid redirections, invalid map names, invalid localvar names etc... More will come with time. Suggestions are welcome, if possible they'll be implemented.
